...
def main():
    ...
    # X, y = generate_linear()
    # #X_train = np.array(X[:50].T)
    # #y_train = np.array(y[:50].T)
    # X_train, y_train = np.array(X[:50].T), np.array(y[:50].T)
    # # print(y_train.shape)
    # # x,y= GenData.fetch_data('Linear', 50)
    # # X=x.T
    # # y=y.T
    # model = SimpleNet((20,7), lr=0.1,num_step=10000,print_interval=1000)
    # # for i in range(10000):
    # #     pred_y = model.forward(X_train)
    # #     model.backward(y_train, pred_y)
    # #     if i % 1000 == 0:
    # #         loss=model.compute_loss(y_train,pred_y)
    # #         acc=(1.-np.sum(np.abs(y_train-np.round(pred_y)))/y_train.shape[1])*100          
    # #         print(f'Epochs {i}: loss={loss:.5f} accuracy={acc:.2f}%')
